...
export const EVAC_TUNE_DEFAULT: EvacTune = {
  ...
  btn: { capColor: "#070a0e", borderColor: "#222730", neutralY: 0.008, inY: -0.04, staysY: -0.014 }, // HYD live-fire defaults
...
function EvacScene({ tune, active, hornSignal, btnPos, onCommand, onHorn, onCapt }: {
  ...
  const faceAlbedo = useMemo(() => {
    ...
    try {
      ...
      for (let p = 0; p < d.length; p += 4) {
        const r = d[p], b = d[p + 2];
        if (b - r > 12 && b > 60) { d[p] = pr; d[p + 1] = pg; d[p + 2] = pb; } // blue field → base colour
      }
      ctx.putImageData(data, 0, 0);
      const tex = new THREE.CanvasTexture(cnv);
      tex.flipY = false;
    ...
    clone.traverse((obj) => {
      ...
      const remap = (m: THREE.Material): THREE.Material => {
        ...
        if (m.name === "Blue base" || m.name === "Blue base.001") {
          ...
          const base = new THREE.MeshPhysicalMaterial({ color: PANEL_BLUE, metalness: 1.5, roughness: 0.6, clearcoat: 0.4, clearcoatRoughness: 0.22, envMapIntensity: 1.0 });
