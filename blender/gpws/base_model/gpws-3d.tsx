...
const LEG_DIM = "#2a2f37", LEG_WHITE = "#f3f6fa", LEG_AMBER = "#ff9f00", LEG_BLUE = "#3aa0ff";
...
const GPWS_ON_COLOR: Record<GpwsBtnKey, string> = { terr: LEG_WHITE, sys: LEG_WHITE, gsMode: LEG_WHITE, flapMode: LEG_WHITE, ldgFlap3: LEG_BLUE };
...
function GpwsScene({ tune, pos, lights }: { tune: GpwsTune; pos: GpwsPos; lights: GpwsLights }) {
  ...
  const { root, groups, movable, worldNormal, legendsByKey } = useMemo(() => {
    ...
    const legWithX = legendEntries.map((e) => ({ mat: e.mat, x: e.mesh.getWorldPosition(new THREE.Vector3()).x })).sort((a, b) => a.x - b.x);
    const cols: THREE.MeshBasicMaterial[][] = []; let cur: THREE.MeshBasicMaterial[] = []; let lx = Infinity;
    legWithX.forEach((e) => { if (cur.length && Math.abs(e.x - lx) > 0.3) { cols.push(cur); cur = []; } if (!cur.length) lx = e.x; cur.push(e.mat); });
    if (cur.length) cols.push(cur);
    const legendsByKey = {} as Record<GpwsBtnKey, THREE.MeshBasicMaterial[]>;
    cols.forEach((col, i) => { const key = GPWS_BTN_ORDER[i]; if (key) legendsByKey[key] = col; });
    return { root: clone, groups: g, movable, worldNormal, legendsByKey };
  ...
  useEffect(() => {
    GPWS_BTN_ORDER.forEach((key) => {
      const st = lights[key] ?? {};
      const fault = !!st.fault && !!GPWS_HAS_FAULT[key];
      const on = !!st.on;
      const lit = fault || on;
      const col = fault ? LEG_AMBER : on ? GPWS_ON_COLOR[key] : tune.legendColor; // FAULT amber > ON/OFF > dim
      (legendsByKey[key] ?? []).forEach((m) => { m.color.set(col); m.opacity = lit ? 1 : 0.55; m.needsUpdate = true; });
    });
...
export function GpwsPanel3D({ tune, pos = "neutral", lights = {} }: { tune?: GpwsTune; pos?: GpwsPos; lights?: GpwsLights }) {
  ...
  return (
    ...
    >
      ...
      <Suspense fallback={null}>
        ...
        <GpwsScene tune={tune ?? GPWS_TUNE_DEFAULT} pos={pos} lights={lights} />
      </Suspense>
