"""
A320 ENG 1 Fire Panel — LEFT SIDE ONLY (for Unreal Engine import)
Standalone extraction of just the ENG1 section from fire_panel_two.
PX = 0.25  (1 CSS pixel = 0.25 mm)

Panel is now 440 × 420 px (110 × 105 mm) — only the left third of the
original 1200 × 420 px three-section panel. All other geometry (FIRE pb +
guard, AGENT 1, AGENT 2, TEST pb, vertical FIRE bar, labels, screws,
materials) is preserved exactly as in the best_version snapshot.

When Unreal needs an FBX, open this .blend, select the meshes you want,
File > Export > FBX (.fbx).
"""
...
PX   = 0.25
W_PX = 440      # ENG1 section width only (was 1200 for the 3-section panel)
H_PX = 420
W    = W_PX * PX
H    = H_PX * PX
...
# Corner screws — repositioned for the 440 × 420 panel
for sx, sy in [(12,12),(416,12),(12,396),(416,396)]:
    cx, cy = p(sx+6, sy+6)
    cyl(f'Scr_{sx}_{sy}', cx, cy, BASE_Z+0.75, 6*PX, 1.5, M['screw'], verts=16)

# FIRE title — moved to center of the new 440-wide panel (was at 600)
cx, cy = p(220, 12)
...
# Only ENG1 — APU and ENG2 sections intentionally omitted for the
# Unreal Engine left-panel extract.
section(0, 440, 'ENG1')
...
sc.render.filepath              = os.path.join(_HERE, 'eng1_left_panel.png')
...
print("DONE: /Users/czar/Desktop/eng1_left_panel/eng1_left_panel.png")
