  ...
  const root = useMemo(() => {
    ...
    clone.traverse((obj) => {
      ...
      const remap = (m: THREE.Material) => {
        if (m.name === "DECALS") {
          // Face = baked Blender albedo driven through EMISSIVE (so scene lights
          // can never darken it — that was the near-black bug), PLUS a metallic
          // layer that reflects the HDRI for Blender-like sheen. Black base so
          // only the emissive albedo + the env reflection show.
          ...
          std.name = "DECALS"; // keep name so the camera-fit can find the face
          return std;
        }
        return m.clone();
      };
